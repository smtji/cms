...
final class Request
{
    ...
    public static function getBody(): ?string
    {
        ...
        if ('POST' === $method || 'PUT' === $method || 'DELETE' === $method || 'PATCH' === $method) {
            $methodInput = fopen('php://input', 'r');
            fseek($methodInput, 0, SEEK_END);
            $tmpLen = ftell($methodInput);
            fseek($methodInput, 0, SEEK_SET);
            if ($tmpLen < 1) {
                $body = null;
                fclose($methodInput);
                return $body;
            }
            $body = fread($methodInput, $tmpLen);
            fclose($methodInput);
        }
